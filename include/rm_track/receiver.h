...
protected:
  std::shared_ptr<Trackers>& allocateTrackers(int id)
  {
    id2trackers_.insert(std::make_pair(
        id, std::make_shared<Trackers>(id, max_match_distance_, max_lost_time_, max_storage_time_, max_new_armor_time_,
                                       max_judge_period_dynamic_, max_follow_distance_dynamic_)));
